...
- [Llama Instance](https://github.com/llamaxyz/llama/raw/main/diagrams/llama-overview.png): A Llama instance is a self-managed cluster of contracts that enables onchain access control over privileged functions. The main parts of an instance are the `Core`, `Policy`, and `Executor` contracts.
...
- **Strategy Logic:** The initial strategy implementation (logic) contract. Look at the [strategy comparison table](/framework/strategies#comparison-table) to learn about their differences.
...
- **Initial Strategies:** An array of initial strategy configurations. All configurations are for the `strategyLogic` defined above. If you'd like to configure other strategies with different logic contracts, that must be done in an action after deployment
  ...
  - Each strategy has a `Config` struct that defines what data is required to initialize the strategy. Each config struct can be unique, but let's look at an example JSON blob which is intended to configure a [relative strategy](https://github.com/llamaxyz/llama/blob/main/src/strategies/relative/LlamaRelativeStrategyBase.sol):
  ...
  - You can read more about what each of these fields mean [here](/framework/strategies)
